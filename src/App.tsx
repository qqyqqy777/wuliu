...
const RuleItem: React.FC<RuleItemProps> = React.memo(
  ({ label, value, limit, unit, valid, isMin }) => (
    <div
      className={`flex items-center justify-between p-2 rounded border-b last:border-b-0 transition-all ${valid ? "border-zinc-100 hover:bg-zinc-50/50/50" : "border-rose-100 bg-rose-50/30"}`}
    >
      <span
        className={`text-[10px] font-black uppercase tracking-tighter truncate mr-2 ${valid ? "text-zinc-400" : "text-rose-400"}`}
        title={label}
      >
        {label}
      </span>
      <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0">
        <span
          className={`font-mono font-bold text-xs ${valid ? "text-zinc-700" : "text-rose-600"}`}
        >
          {value}{" "}
          <span className="text-[9px] font-normal opacity-40">{unit}</span>
        </span>
        <div className={`w-px h-3 ${valid ? "bg-zinc-200" : "bg-rose-200"}`} />
        <span
          className={`text-[9px] font-mono font-bold min-w-[40px] sm:min-w-[45px] text-right ${valid ? "text-zinc-400" : "text-rose-500"}`}
        >
          {isMin ? "MIN" : "MAX"}:{limit}
        </span>
      </div>
      ...
      <div className="p-6 sm:p-8 space-y-8">
          ...
          <div
            className={`border-2 border-dashed rounded-xl p-8 text-center transition-colors cursor-pointer ${
              fileData
                ...
                : "border-zinc-300 hover:border-indigo-400 bg-zinc-50/50"
            }`}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <button
            ...
            className="bg-zinc-900 hover:bg-zinc-800 disabled:opacity-50 disabled:cursor-not-allowed text-white shadow-sm ring-1 ring-zinc-900/10 font-bold py-4 rounded-xl flex items-center justify-center gap-2 transition-all shadow-md active:scale-[0.99]"
          >
            {loading ? (
          ...
          <button
            ...
            className="bg-emerald-600 hover:bg-emerald-500 shadow-sm ring-1 ring-emerald-600/20 disabled:opacity-50 disabled:cursor-not-allowed text-white font-bold py-4 rounded-xl flex items-center justify-center gap-2 transition-all shadow-md active:scale-[0.99]"
          >
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-zinc-50/50/50 font-sans text-zinc-900 selection:bg-indigo-100 selection:text-indigo-900">
          ...
          <div className="py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-center gap-3 text-zinc-900">
              ...
              <div>
                ...
                <p className="text-zinc-500 text-xs mt-0.5 font-medium tracking-wide">
                  渠道自动寻优 / 产品资料入库 / 订单库龄路由
                </p>
              </div>
            </div>

            <nav className="flex bg-zinc-100/80 p-1 rounded-xl border border-zinc-200/50 items-center justify-between shadow-inner">
              <div className="flex gap-1">
                <button
                  onClick={() => setActiveTab("query")}
                  className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-all ${activeTab === "query" ? "bg-white text-zinc-900 shadow-sm ring-1 ring-zinc-200" : "text-zinc-600 hover:text-zinc-900 hover:bg-zinc-200/50"}`}
                >
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold text-rose-600 hover:text-white hover:bg-rose-500 transition-all"
                title="清空所有上传底表与缓存"
              >
              ...
              <div className="mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div
                    className={`border-2 border-dashed rounded-xl p-6 text-center transition-all cursor-pointer relative group ${
                      csvData
                        ...
                        : "border-zinc-300 hover:border-indigo-400 bg-zinc-50/50 hover:bg-indigo-50/30"
                    } overflow-hidden`}
                    onClick={() => fileInputRef.current?.click()}
                  >
                    <div className="absolute top-0 left-0 bg-zinc-800 text-white text-[10px] px-2 py-1 rounded-br-lg rounded-tl-xl font-bold uppercase tracking-wider z-10">
                      Required 必需
                    </div>
                    ...
                    {loading ? (
                      ...
                      <div className="flex flex-col items-center py-2 text-zinc-500 hover:text-indigo-600 transition-colors relative z-10">
                        <TableProperties className="w-10 h-10 mb-3 text-zinc-400 group-hover:text-indigo-500 transition-colors" />
                        <p className="font-bold text-zinc-700 mb-1 group-hover:text-indigo-700">
                          导入尾程报价表
                        </p>
                        <p className="text-xs text-zinc-500 group-hover:text-indigo-600/70">
                          支持 Excel / CSV 利润表结构
                        </p>
                      </div>
                    )}
                  ...
                  <div
                    className={`border-2 border-dashed rounded-xl p-6 text-center transition-all cursor-pointer relative group ${
                      Object.keys(priorityMap).length > 0
                        ...
                        : "border-zinc-300 hover:border-sky-400 bg-zinc-50/50 hover:bg-sky-50/30"
                    } overflow-hidden`}
                    onClick={() => priorityInputRef.current?.click()}
                  >
                    <div className="absolute top-0 left-0 bg-zinc-200 text-zinc-600 text-[10px] px-2 py-1 rounded-br-lg rounded-tl-xl font-bold uppercase tracking-wider z-10">
                      Optional 选填
                    </div>
                    ...
                    {priorityLoading ? (
                      ...
                      <div className="flex flex-col items-center py-2 text-zinc-500 hover:text-sky-600 transition-colors relative z-10">
                        <Database className="w-10 h-10 mb-3 text-zinc-400 group-hover:text-sky-500 transition-colors" />
                        <p className="font-bold text-zinc-700 mb-1 group-hover:text-sky-700">
                          导入发货与库存文件
                        </p>
                        <p className="text-xs text-zinc-500 group-hover:text-sky-600/70">
                          支持读取真实【可用库存】与优先规则
                        </p>
                      </div>
                    )}
                  ...
                  <div
                    className={`border-2 border-dashed rounded-xl p-6 text-center transition-all cursor-pointer relative group ${
                      Object.keys(productInfoMap).length > 0
                        ...
                        : "border-zinc-300 hover:border-amber-400 bg-zinc-50/50 hover:bg-amber-50/30"
                    } overflow-hidden`}
                  ...
                  >
                    ...
                    <button
                      className="absolute top-2 right-2 text-zinc-400 hover:text-amber-600 transition-colors z-20 flex items-center gap-1 text-[10px] font-bold bg-white/50 px-2 py-1 rounded-full shadow-sm"
                    ...
                    {productLoading ? (
                      ...
                      <div className="flex flex-col items-center py-2 text-zinc-500 hover:text-amber-600 transition-colors relative z-10">
                        <Package className="w-10 h-10 mb-3 text-zinc-400 group-hover:text-amber-500 transition-colors" />
                        <p className="font-bold text-zinc-700 mb-1 group-hover:text-amber-700">
                          导入产品箱规表
                        </p>
                        <p className="text-xs text-zinc-500 group-hover:text-amber-600/70">
                          支持识别 SKU / 一箱件数
                        </p>
                      </div>
                    )}
                ...
                <div className="mb-8 p-5 bg-indigo-50/50 border border-indigo-100 rounded-xl flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <select
                      ...
                      className="w-full bg-white border border-indigo-200 text-zinc-900 font-medium text-sm rounded-xl focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 focus:border-indigo-500 block p-2.5 outline-none shadow-sm transition-all cursor-pointer"
                    >
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-4">
                  ...
                  <div className="flex items-center space-x-2">
                    <input
                      ...
                      className="w-4 h-4 text-indigo-600 bg-zinc-100 border-zinc-300 rounded focus:ring-indigo-500 cursor-pointer"
                    />
                    <label
                      htmlFor="eu-account-toggle"
                      className="text-sm font-semibold text-zinc-700 flex items-center cursor-pointer"
                    >
                      <Globe className="w-4 h-4 text-zinc-400 mr-1.5" />{" "}
                      此订单属于【欧盟账号】
                    </label>
                  </div>
                </div>
                ...
                >
                  <div className="flex-1 w-full">
                    <label className="block text-[11px] font-semibold text-zinc-500 uppercase tracking-widest mb-2 flex items-center gap-1.5">
                  ...
                  <button
                    type="submit"
                    className="w-full sm:w-auto bg-zinc-900 hover:bg-zinc-800 text-white font-bold py-3.5 px-8 rounded-xl flex items-center justify-center gap-2 transition-all shadow-md hover:shadow-lg active:scale-[0.99] text-sm"
                  >
              ...
              {result && (
                <div className="animate-in fade-in slide-in-from-bottom-4 space-y-6 mt-8">
                  <div className="flex items-center gap-3 px-2">
                    ...
                    <span className="text-xs font-bold text-zinc-400 uppercase tracking-widest bg-zinc-50/50 px-2">
                      调度中心报告
                    </span>
                    <div className="h-px bg-zinc-200 flex-1"></div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                    {/* ====== 首选方案卡片 ====== */}
                    <div
                      className={`lg:col-span-8 bg-white rounded-xl shadow-sm border overflow-hidden relative ${
                      ...
                      <div className="p-8">
                        <div className="flex items-center justify-between mb-6 relative z-10">
                          <div className="flex items-center gap-2">
                            ...
                            <span
                              className={`text-xs font-bold ${result.recommendType === "eu-blocked" || result.recommendType === "no-stock" ? "text-rose-600" : "text-zinc-500"}`}
                            >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
                          <div>
                            ...
                            <h3
                              className={`text-4xl font-black mb-6 ${result.recommendType === "eu-blocked" || result.recommendType === "no-stock" ? "text-rose-600" : "text-zinc-900"}`}
                            >
                            ...
                            <div className="space-y-4">
                              <div className="flex items-center justify-between border-b border-zinc-100 pb-2">
                                ...
                                <div className="flex flex-wrap items-center justify-end gap-1.5 font-bold text-zinc-900 flex-1 ml-4 shadow-sm-border">
                          ...
                          <div
                            className={`rounded-xl p-6 border flex flex-col justify-center transition-all ${
                              ...
                              result.recommendType === "no-stock"
                                ...
                                : "bg-zinc-50/50 border-zinc-200/60 hover:border-indigo-300 hover:bg-indigo-50/20 cursor-pointer shadow-sm hover:shadow-md"
                            }`}
                            onClick={() => {
                              if (
                                result.recommendType !== "eu-blocked" &&
                          ...
                          >
                            <p className="text-xs font-bold text-zinc-400 uppercase tracking-widest mb-2 text-center">
                              系统优选渠道
                            </p>
                            <p
                            ...
                            <div className="flex flex-col items-center">
                              ...
                              <p className="text-4xl font-black text-zinc-900 flex items-baseline gap-1">
                        ...
                        {showBestOptionDetails &&
                          ...
                          result.bestOption.details.length > 0 && (
                            <div className="mt-6 pt-5 border-t border-zinc-100 animate-in fade-in relative z-10">
                              ...
                              <div className="bg-zinc-50/50 rounded-xl border border-zinc-100 p-3 max-h-64 overflow-y-auto space-y-1.5 shadow-inner">
                                {result.bestOption.details.map(
                                  (detail: any, idx: number) => (
                                    <div
                                      key={idx}
                                      className="flex justify-between items-center py-2 px-4 hover:border-indigo-200 bg-white border border-zinc-100 rounded-xl transition-colors gap-4 shadow-sm hover:shadow"
                                    >
                                      <span className="text-[13px] font-medium text-zinc-700 flex-1 whitespace-normal break-words leading-tight">
                                        {detail.channel}
                                      </span>
                                      <span
                                        className={`text-[15px] font-bold whitespace-nowrap ${idx === 0 ? "text-emerald-600" : "text-zinc-900"}`}
                                      >
                      ...
                      <div className="bg-white rounded-xl shadow-sm border border-zinc-200 p-6">
                        ...
                        <div className="space-y-3">
                          <div className="flex items-center justify-between border-b border-zinc-100 pb-2">
                            ...
                            <span
                              className={`font-mono text-sm font-semibold ${result.hasFullDims ? "text-zinc-900" : "text-zinc-400 italic"}`}
                            >
                              {result.dims}
                            </span>
                          </div>
                          ...
                          <div
                            className={`flex items-center justify-between p-3 rounded-xl mt-3 ${result.hasQty ? "bg-indigo-50/70 border border-indigo-100/50" : "bg-zinc-50/50 border border-zinc-100"}`}
                          >
                            <span
                              className={`text-sm font-medium flex items-center gap-1.5 ${result.hasQty ? "text-indigo-700" : "text-zinc-500"}`}
                            >
                              <Layers
                                className={`w-4 h-4 ${result.hasQty ? "text-indigo-500" : "text-zinc-400"}`}
                              />{" "}
                              一箱装量
                            </span>
                            <span
                              className={`font-mono text-base font-bold ${result.hasQty ? "text-indigo-700" : "text-zinc-400 italic"}`}
                            >
                              {result.hasQty
                                ? `${result.qtyVal} 件/箱`
                                : "未配置 (默认 1)"}
                        ...
                        {result.hasFullDims && (
                          <button
                            ...
                            className="mt-4 w-full bg-zinc-900 hover:bg-zinc-800 text-white py-2.5 rounded-xl text-xs font-bold transition-colors flex justify-center items-center gap-2 shadow-sm relative overflow-hidden group"
                          >
                          ...
                          <div className="space-y-2 max-h-[500px] overflow-y-auto pr-1">
                            {result.altOptions.map((opt: any, idx: number) => {
                              ...
                              return (
                                <div
                                  key={idx}
                                  className={`border rounded-xl transition-all overflow-hidden ${isBlocked ? "bg-zinc-50/50 border-zinc-100 opacity-60 grayscale-[0.3]" : "border-zinc-200 hover:border-indigo-300 hover:shadow-sm"}`}
                                >
                                  <div
                                    className={`flex items-start justify-between p-4 ${isBlocked ? "cursor-pointer hover:bg-zinc-100" : "cursor-pointer hover:bg-indigo-50/20"}`}
                                    onClick={() =>
                                      setExpandedAltWH(
                                  ...
                                  >
                                    <div className="flex-1 min-w-0 pr-3 flex flex-col justify-center">
                                      <div className="flex items-center gap-2 flex-wrap mb-1.5">
                                        <p className="font-bold text-[15px] text-zinc-900 tracking-tight leading-none">
                                          {opt.warehouse}
                                        </p>
                                        {isEUBlocked && (
                                        ...
                                        {isNoStock && (
                                          <span className="text-[10px] text-zinc-500 font-bold border border-zinc-200 px-1.5 py-0.5 rounded-md bg-zinc-100 shadow-sm leading-none">
                                            无货
                                          </span>
                                        )}
                                      </div>
                                      ...
                                      <p className="text-[11px] text-zinc-500 break-words whitespace-normal leading-tight font-medium opacity-80">
                                        {opt.channel}
                                      </p>
                                    </div>
                                    <div className="text-right flex items-center justify-center gap-3 flex-shrink-0 self-center">
                                      <p className="font-extrabold text-lg text-zinc-900 tracking-tight leading-none">
                                        €{opt.costNum.toFixed(2)}
                                      </p>
                                      <div className="text-zinc-400 bg-white border border-zinc-200 rounded-md p-1 shadow-sm transition-transform hover:bg-zinc-50/50">
                                        {isExpanded ? (
                                          <svg
                                            className="w-4 h-4"
                                            fill="none"
                                  ...
                                  {isExpanded && opt.details && (
                                    <div className="bg-zinc-50/50 border-t border-zinc-200 p-3 space-y-1.5 shadow-inner">
                                      {opt.details.map(
                                        (detail: any, dIdx: number) => (
                                          <div
                                            key={dIdx}
                                            className="flex justify-between items-center py-2 px-3 bg-white border border-zinc-100 hover:border-indigo-200 rounded-xl transition-colors text-xs gap-3 shadow-sm hover:shadow"
                                          >
                                            <span className="font-medium text-zinc-700 break-words whitespace-normal flex-1 leading-tight">
                                              {detail.channel}
                                            </span>
                                            <span
                                              className={`font-bold whitespace-nowrap text-sm ${dIdx === 0 ? "text-emerald-600" : "text-zinc-900"}`}
                                            >
                                              €{detail.costNum.toFixed(2)}
                                            </span>
                                          </div>
                                      ...
                                      {opt.details.length === 0 && (
                                        <p className="text-xs text-center text-zinc-400 py-3 font-medium">
                                          无其他渠道明细
                                        </p>
                                      )}
                                    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              <div className="lg:col-span-4 space-y-6">
                <div className="bg-white p-6 rounded-xl shadow-sm border border-zinc-200">
                  <h3 className="text-sm font-black mb-5 flex items-center text-zinc-900 uppercase tracking-wider">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full bg-zinc-50/50 border border-zinc-200 rounded-xl px-4 py-2.5 text-sm font-mono focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 focus:bg-white transition-all shadow-inner"
                      />
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                      <div>
                        ...
                        <input
                          ...
                          value={inputs.length}
                          onChange={handleCalcInputChange}
                          className="w-full font-mono text-sm bg-zinc-50/50 border border-zinc-200 rounded-xl px-3 py-2.5 focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 focus:bg-white shadow-inner"
                        />
                      </div>
                      <div>
                        <label className="block text-[11px] font-semibold text-zinc-500 uppercase tracking-widest mb-1.5">
                  ...
                  <div className="space-y-4">
                    <div>
                      <select
                        ...
                        className="w-full bg-zinc-50/50 border border-zinc-200 text-sm font-medium rounded-xl px-4 py-3 focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 cursor-pointer shadow-sm"
                      >
                        <option value="min">
                ...
                <div className="bg-zinc-900 p-6 rounded-xl shadow-xl text-white relative overflow-hidden group border border-zinc-800">
                  ...
                  <h3 className="text-xs font-bold text-zinc-400 uppercase tracking-widest mb-5 flex items-center relative z-10">
                  ...
                  <div className="flex justify-between items-center relative z-10 bg-white/5 p-4 rounded-xl border border-white/10 backdrop-blur-sm">
                    <div className="text-center flex-1">
                      <p className="text-[9px] text-zinc-400 uppercase font-bold tracking-widest mb-1">
                        DHL
                      </p>
                    ...
                    <div className="h-8 border-r border-zinc-700 mx-1"></div>
                    <div className="text-center flex-1">
                      <p className="text-[9px] text-zinc-400 uppercase font-bold tracking-widest mb-1">
                        GLS
                      </p>
              ...
              <div className="lg:col-span-8 space-y-6">
                <div className="bg-white p-6 sm:p-8 rounded-xl shadow-sm border border-zinc-200 grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8">
                  <div className="flex flex-col justify-center text-center md:text-left">
                    <p className="text-[10px] font-black text-zinc-400 uppercase tracking-widest mb-2 flex items-center justify-center md:justify-start gap-1">
                      <Package className="w-3 h-3" /> 成箱物理尺寸
                    </p>
                    <div className="text-xl sm:text-2xl font-black text-zinc-900 flex items-baseline justify-center md:justify-start font-mono tracking-tight">
                      {calcData.maxSide}
                      <span className="text-zinc-300 text-sm mx-1 font-normal">
                        ×
                      </span>
                      {calcData.midSide}
                      <span className="text-zinc-300 text-sm mx-1 font-normal">
                      ...
                      {calcData.minSide}
                      <span className="text-zinc-400 text-sm ml-1.5 font-bold">
                        cm
                      </span>
                    </div>
                  </div>

                  <div className="flex flex-col justify-center text-center md:text-left md:border-l md:border-zinc-100 md:pl-8">
                    <p className="text-[10px] font-black text-zinc-400 uppercase tracking-widest mb-2 flex items-center justify-center md:justify-start gap-1">
                      <Scale className="w-3 h-3" /> 成箱实重计费
                    </p>
                    <div className="text-2xl font-black text-zinc-900 flex items-baseline justify-center md:justify-start font-mono">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  {[
                    ...
                    <div
                      key={id}
                      className={`relative overflow-hidden rounded-xl border-2 transition-all duration-300 ${data.isValid ? "bg-white border-zinc-200 shadow-sm" : "bg-zinc-50/50 border-rose-200"}`}
                    >
                      {/* 头部：系统状态指示器 */}
                      <div
                        className={`flex items-center justify-between px-4 py-2.5 border-b ${data.isValid ? "bg-zinc-50/50 border-zinc-100" : "bg-rose-50 border-rose-100"}`}
                      >
                        <div className="flex items-center gap-2">
                          <div
                      ...
                      <div className="p-4 space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          <div className="p-2.5 bg-zinc-50/50 rounded border border-zinc-100">
                        ...
                        {data.surcharges && data.surcharges.length > 0 && (
                          <div className="flex flex-col gap-1 pt-2 border-t border-zinc-100 mt-2">
                ...
                <div className="bg-white p-6 rounded-xl shadow-sm border border-zinc-200">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div className="space-y-2">
                              ...
                              {sorted.length > 1 && (
                                <div className="flex items-center gap-2 opacity-60">
                                  <span className="bg-zinc-400 text-white text-[10px] px-2 py-0.5 rounded font-black">
                                    备选
                                  </span>
                    ...
                    <div className="p-4 bg-zinc-50/50 rounded-xl border border-zinc-100">
                        <p className="text-xs font-bold text-zinc-500 mb-3 uppercase tracking-widest">
                        GLS/DPD 5.15规则摘要
                      </p>
                      <div className="space-y-3">
                        <div className="flex justify-between text-[10px] border-b border-zinc-200 pb-1.5">
                          <span className="text-zinc-400">GLS体积附加费阈值</span>
                          <span className="font-bold text-zinc-700">
                            0.15 m³ (约 {FREIGHT_FEES.GLS.volume} RMB)
                          </span>
                        </div>
                        <div className="flex justify-between text-[10px] border-b border-zinc-200 pb-1.5">
                          <span className="text-zinc-400">GLS超长附加费</span>
                          <span className="font-bold text-zinc-700">
                            &gt;150 cm (约 {FREIGHT_FEES.GLS.overLength} RMB)
                          </span>
                        </div>
                        <div className="flex justify-between text-[10px] border-b border-zinc-200 pb-1.5">
                          <span className="text-zinc-400">DPD超重附加费</span>
                          <span className="font-bold text-zinc-700">
                            31.5-40 kg (约 {FREIGHT_FEES.DPD.overweight} RMB)
                          </span>
                        </div>
                        <div className="flex justify-between text-[10px] border-b border-zinc-200 pb-1.5">
                          <span className="text-zinc-400">极度超限罚金</span>
            ...
            <div className="p-5 overflow-y-auto flex-1">
              <div className="mb-4 flex items-center justify-between text-sm">
                <span className="font-bold text-zinc-600">
                  SKU:{" "}
                  <span className="font-mono text-zinc-900">{result.sku}</span>
                </span>
                <span className="font-bold text-zinc-600">
                  流向: <span className="text-zinc-900">{result.country}</span>
                </span>
              </div>
              <div className="space-y-2">
                {result.allCosts.map((c: any, idx: number) => (
                  <div
                    key={idx}
                    className="flex items-center justify-between p-3 rounded-xl border border-zinc-100 bg-zinc-50/50 hover:bg-indigo-50 hover:border-indigo-100 transition-colors"
                  >
                    <div className="flex items-center gap-3">
                      <span className="w-6 h-6 rounded-full bg-zinc-200 text-zinc-500 flex items-center justify-center text-xs font-black">
                        {idx + 1}
                      </span>
                      <span className="font-bold text-zinc-900">
                        {c.channel}
